...
const createOrder = async (req, res) => {
    ...
    try {
        const newOrder = await OrderSchema_1.default.create({
            user,
            resturent,
            cartItems,
            totalPrice,
...
const getOrders = async (req, res) => {
    ...
    try {
        const orders = await OrderSchema_1.default.find({ user: userId }).populate({
            path: "resturent",
...
const updateOrderStatus = async (req, res) => {
    ...
    try {
        const order = await OrderSchema_1.default.findById(id);
        if (!order) {
            return res
                .status(404)
